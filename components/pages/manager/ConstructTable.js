...
const ConstructTable = ({
  ...
  const renderCell = (data, columnKey) => {
    ...
    switch (columnKey) {
      case "createDate":
        const showDate = new Date(cellValue);

        return <div>{`${showDate.getDate()}/${showDate.getMonth()+1}/${showDate.getFullYear()}`}</div>;
            ...
            <Col css={{ d: "flex" }}>
              <Tooltip content="Details">
                <IconButton
                  onClick={() => {
                    setIsLoad(true);
                    router.push("/Manager/Formula/Detail");
                  }}
                >
