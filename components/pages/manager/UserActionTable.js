...
export default function UserActionTable({
  ...
  const handleDelete = (user) => {
    if (deletePath == "/manager") {
      if (session.id == user.userID_usersProducer) {
        Swal.fire({
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        console.log(process.env.NEXT_PUBLIC_API_URL + "/user" + deletePath);
        // api delete
        const sendData = { id: user.userID_usersProducer };
        const res = await fetch(
          process.env.NEXT_PUBLIC_API_URL + "/user" + deletePath,
          {
  ...
  const renderCell = (data, columnKey) => {
    ...
    switch (columnKey) {
      case "createDate":
        const dateData = new Date(data.createDate);
        ...
        return <Col>{dd}/{mm}/{yyyy} {hours}:{minutes}:{seconds}</Col>;
      case "userid":
