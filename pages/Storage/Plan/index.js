...
const Plan = ({ data }) => {
  ...
  const startDate = new Date(data.APIdata_nowPlan.data.startDate);
  const startDate_yyyy = startDate.getFullYear();
  let startDate_mm = startDate.getMonth() + 1;
  let startDate_dd = startDate.getDate();

  const endDate = new Date(data.APIdata_nowPlan.data.endDate);
  ...
  return (
    <Layout session={data.session}>
      ...
      <button class="mb-3 bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded"
        onClick={() =>{router.push({
      ...
      <div className="flex flex-col gap-3">
        <div className="border drop-shadow bg-gray-100 rounded-lg p-5 flex flex-row flex-wrap gap-3">
          <div className=" flex flex-wrap flex-row flex-grow  min-w-[200px]">
            ...
            [{data.APIdata_nowPlan.data.planID}] &nbsp; &nbsp;
            </p>
            <p>
              {data.APIdata_nowPlan.data.PlanName}
            </p>
          </div>
        ...
        {data.APIdata_nowPlan.data.lotList.map((_lots, index) => (
          <div
            key={`plan_${data.APIdata_nowPlan.data.planID}_lot_${_lots.LotID}`}
            className="ml-3"
          >
